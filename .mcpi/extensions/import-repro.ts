/**
 * Import an mcpi session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://github.com/SamMorrowDrums/mcpi/issues/123
 *
 *   mcpi "/ir <gist-id>"
 */
...
import type { ExtensionAPI, ExtensionCommandContext } from "@sammorrowdrums/mcpi";
...
function parseRef(
	...
	throw new Error(
		`expected a gist ID, gist URL, upstream pi.dev share URL, issue URL, .html file, or .jsonl file: ${ref}`,
	);
}
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded mcpi session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded mcpi session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded mcpi session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded mcpi session data has no entries array");
	}
...
function getCiWorkdirName(sourceCwd: string): string | undefined {
	...
	return /^mcpi-ci-[0-9a-f]{32}$/i.test(name) ? name : undefined;
}
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("ir", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			const ref = args.trim();
			if (!ref) {
				ctx.ui.notify("Usage: /ir <gist-id | gist-url | upstream pi.dev/session URL | issue URL>", "error");
				return;
			}
