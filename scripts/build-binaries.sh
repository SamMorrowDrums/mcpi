#!/usr/bin/env bash
#
# Build mcpi binaries for all platforms locally.
...
#     mcpi-darwin-arm64.tar.gz
#     mcpi-darwin-x64.tar.gz
#     mcpi-linux-x64.tar.gz
#     mcpi-linux-arm64.tar.gz
#     mcpi-windows-x64.zip
#     mcpi-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before mcpi starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/mcpi.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/mcpi"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating mcpi-$platform.zip..."
        (cd "$platform" && zip -r ../mcpi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating mcpi-$platform.tar.gz..."
        mv "$platform" mcpi && tar -czf mcpi-$platform.tar.gz mcpi && mv mcpi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../mcpi-$platform.zip)
    else
        tar -xzf mcpi-$platform.tar.gz && mv mcpi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/mcpi.exe"
    else
        echo "  $OUTPUT_DIR/$platform/mcpi"
    fi
done
