...
test_root="$(mktemp -d "$temp_parent/mcpi-test.XXXXXX")"
git_askpass="$(type -P false)"
readonly temp_parent test_root git_askpass

mkdir -p "$test_root/home/.config" "$test_root/state" "$test_root/tmp" "$test_root/cache/npm"
# Mark the generated root so cleanup can verify ownership before deleting it.
touch "$test_root/.mcpi-test-owned" "$test_root/npm-userconfig" "$test_root/npm-globalconfig"
...
cleanup() {
	...
	case "$test_root" in
		"$temp_parent"/mcpi-test.*)
			if [[ -d "$test_root" && ! -L "$test_root" && -f "$test_root/.mcpi-test-owned" ]]; then
				rm -rf -- "$test_root"
			else
