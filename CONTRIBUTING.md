...
First things first: **mcpi's core is minimal**.

If your feature does not belong in the core, it should be an extension. PRs that bloat the core will likely be rejected.

mcpi's core exists to be minimal and to be extensible so that it can be influenced and manipulated by extensions.  Even hook points for extensions however should be well considered and discussed to avoid adding unmaintainable bloat and complex interactions.
...
If you use an agent, run it from the `mcpi` root directory so it picks up `AGENTS.md` automatically. Your agent must follow the rules and guidelines in that file.
...
For upstream pi's longer-term plans, see Earendil's public
[RFCs](https://rfc.earendil.com/keyword/pi/). Plans specific to mcpi are tracked
in this repository.
