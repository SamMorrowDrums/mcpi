...
- Enabled native deferred tool loading on GitHub Copilot's Anthropic gateway. `defer_loading` and `tool_reference` were gated on `provider === "anthropic"`, so Copilot-served Claude models always sent every tool definition inline. A live probe on `github-copilot/claude-opus-5` with 12 tools measured 11,885 input tokens inline against 1,352 with 11 tools deferred. Support now comes from probe-verified generated metadata for the five Copilot Claude ids currently routed through `anthropic-messages`: `claude-opus-5`, `claude-sonnet-5`, `claude-opus-4.8`, `claude-opus-4.7`, and `claude-haiku-4.5`. `claude-sonnet-4.6` rejects the protocol, while `claude-fable-5.1` currently uses OpenAI Completions, so neither receives this Anthropic capability.
