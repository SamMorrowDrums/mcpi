...
- Added Anthropic's server-side tool search (`tool_search_tool_bm25_20251119`) to `anthropic-messages` requests that carry deferred tools, so a tool registered with `deferred: true` that no skill ever names can still be found. The model searches its withheld catalog, the server returns `tool_reference` blocks for the matches, only those schemas load, and the tool is then callable directly. Skills remain the proactive push; search is the reactive fallback. The entry is appended after the deferred definitions, past the cache breakpoint, so the cached prefix is unchanged. BM25 is used rather than the regex variant because a regex matching nothing returns no references, which would reproduce the unreachable-tool bug it exists to prevent.
- Added `compat.supportsToolSearch` for `anthropic-messages`. It defaults to on and is only consulted where `supportsToolReferences` is on, because Anthropic only loads a deferred schema through a `tool_reference` returned from search. Setting it to `false` covers an endpoint that resolves references but rejects the search tool, and also disables registration-time deferral there.
- Added a single process-lifetime downgrade for an endpoint that rejects the deferred-tool protocol at runtime. A 400 naming `defer_loading`, the `tool_reference` block type, or the `tool_search_tool_*` type *and* reporting it as unknown, extra, or unsupported disables native handling for that provider, model id, and base URL, records a `deferred_tools_rejected` diagnostic carrying the verbatim API error, and retries once with the full tool list. An unresolvable reference such as `Tool reference 'x' not found in available tools` is excluded, because it proves the endpoint implements references and indicates a client-side activation or naming bug that must stay visible. Every other failure, and the original error text and status, propagate unchanged.
- Added `compat.supportsToolReferences` to the documented `anthropic-messages` compatibility fields. It was already honored but undocumented, and overrides both the catalog and the version heuristic in either direction.
- Added OpenAI's hosted tool search to `openai-responses` and `openai-codex-responses`, so `Tool.deferred` now withholds schemas from turn zero on those APIs instead of expanding them. A request carrying deferred tools sends `defer_loading: true` on those definitions and appends `{ type: "tool_search" }` after them, past the cacheable prefix. The model searches the withheld catalog, the server returns a `tool_search_output` carrying only the matches, and the tool is then callable directly. Gated on the existing `compat.supportsToolSearch` allowlist, which covers `gpt-5.4` and later; every other model keeps sending schemas up front with the `deferred_tools_unsupported` diagnostic.
- Added `Tool.namespace`, an optional `{ name, description }` group a caller attaches to a deferred tool. Tools sharing a namespace are sent as one `{ type: "namespace" }` entry on `openai-responses` and `openai-codex-responses`. This is what makes deferral actually hide a tool there: a flat deferred function still shows the model its name and description and withholds only the parameter schema, while a namespace shows only the group summary. Group names are sanitized to `[a-zA-Z0-9_-]` with deterministic collision suffixes in registration order, so the serialized array is byte-stable across turns and the prompt cache holds. Namespaces are ignored on APIs that do not model them.
- Added `AssistantMessage.toolSearchSteps`, recording each tool search a response performed and the tool names it loaded. mcpi sends `store: false` and rebuilds the whole input every turn, so a search that is not persisted in the transcript is lost; the recorded step is replayed as a `tool_search_call` and `tool_search_output` pair immediately before the first tool call of that message, which is what keeps a searched-for call resolvable on later turns. Only names are stored, so schemas rebuild from the current `Context.tools` and an entry naming a tool that is no longer registered is dropped rather than replayed.
- Added a `deferred_tools_loaded` diagnostic naming the tools a tool search revealed and whether the server or the client ran it, so reactive discovery is observable rather than inferred from token counts. Marker loads are not reported through it: they are already in the transcript as `addedToolNames`, and they are applied while the request is built, so there is no assistant message of their own to carry the diagnostic.
- Added the same single process-lifetime rejection downgrade to `openai-responses`. A 400 naming `defer_loading`, `tool_search`, or `namespace` as unknown, extra, or unsupported disables native handling for that provider, model id, and base URL, records `deferred_tools_rejected` with the verbatim API error, and retries once with the full tool list. An unresolvable tool such as `Tool 'x' not found in namespace 'y'` is excluded, because it proves the endpoint implements the protocol and points at a client-side naming bug that must stay visible. `openai-codex-responses` has no downgrade: exactly one provider serves it, so no gateway can claim the capability falsely.
...
- Restricted `Tool.deferred` to APIs that can withhold a schema without also withholding the tool's name. The Chat Completions Kimi path loads tools from an item anchored to a tool result and exposes no server-side catalog to search, so a tool withheld on turn zero would appear nowhere in the request and be undiscoverable even though it is still registered and dispatchable. It now sends registration-deferred schemas up front with a `deferred_tools_unsupported` diagnostic, and keeps them up front for the rest of the session rather than moving an already-sent schema to a later load point and churning the cached prefix. Deferral driven purely by `addedToolNames` is unchanged on every API.
- Preferred hosted tool search over `additional_tools` on the OpenAI Responses paths when a model supports both. `additional_tools` declares a tool that is *not* in the top-level array; once registration deferral declares the whole deferred catalog up front, re-declaring the same tool there would list it twice, and a tool already in the catalog is loaded by a `tool_search_output` instead.
- Taught `splitDeferredTools` to treat a recorded tool search as a load point. Promotion exists so a tool the model called before any load point becomes immediate for the session; without this, the first call a hosted search enabled would look unexplained, pull that schema into the up-front array, and undo deferral on the turn it started working.
- Extended the `deferred_tools_unsupported` diagnostic on the Anthropic path to cover tools deferred by a `ToolResultMessage.addedToolNames` marker, not only registration-deferred ones, so no expanded schema goes unreported.
- Opted `anthropic-messages` into `providesToolSearch` wherever it sends the search tool, so a tool set that is entirely deferred stays deferred instead of hitting the all-deferred safety floor. That floor exists to guarantee the model has somewhere to start, and the search tool is somewhere to start; the shape it was expanding wholesale, an all-MCP-proxy tool set with no built-ins, is the one deferral helps most.
- Gated registration-time deferral on `anthropic-messages` behind search availability. A schema withheld from turn zero has no transcript position to be loaded at, so where search is unavailable those schemas are sent up front with a `deferred_tools_unsupported` diagnostic instead of being withheld with no way back. Marker-driven deferral is unaffected, since it anchors to a tool result.

### Fixed

- Fixed registration-deferred tools being unreachable when no skill loaded them. A tool registered with `deferred: true` was withheld on turn zero, and the only load point mcpi emitted was a `tool_reference` injected from a `ToolResultMessage.addedToolNames` marker, so a tool no skill named stayed invisible for the entire session. A live probe on `github-copilot/claude-opus-5` reproduced it exactly: asked to use the tool, the model answered that no such tool was available. Offering the server-side search tool alongside the deferred definitions closes the gap, verified on all five Copilot Claude models that route through `anthropic-messages`.
- Enabled native deferred tool loading on GitHub Copilot's Anthropic gateway. `defer_loading` and `tool_reference` were gated on `provider === "anthropic"`, so Copilot-served Claude models always sent every tool definition inline. A live probe on `github-copilot/claude-opus-5` with 12 tools measured 11,885 input tokens inline against 1,352 with 11 tools deferred. Support now comes from a probe-verified allowlist in the generated model catalog covering `claude-opus-5`, `claude-sonnet-5`, `claude-opus-4.8`, `claude-opus-4.7`, `claude-fable-5.1`, and `claude-haiku-4.5`; `claude-sonnet-4.6` uses the same API but rejects the protocol, so it is excluded, as is any id not yet probed.
- Fixed the Anthropic model version parser to accept dotted minor versions. `claude-opus-4.8` and `claude-fable-5.1` failed a dash-only pattern and were treated as pre-4.5, disabling deferred loading independently of the provider gate. First-party Anthropic behavior, including dated snapshot ids such as `claude-opus-4-5-20250929`, is unchanged.
...
- Fixed Cloudflare AI Gateway catalog generation when upstream metadata omits Workers AI passthrough models.
...
- Added explicit `TelemetryContext` propagation across stream, deferred, and image request options using the vendor-neutral `@sammorrowdrums/mcpi-telemetry` contract.
...
- The root entrypoint (`@sammorrowdrums/mcpi-ai`) is now core-only and side-effect free. The old global API moved to the temporary `@sammorrowdrums/mcpi-ai/compat` entrypoint, a strict superset of the root: switching a file's import path is the only migration step. Moved symbols include `stream`/`complete`/`streamSimple`/`completeSimple`, `getModel`/`getModels`/`getProviders` (now deprecated aliases of `getBuiltinModel`/`getBuiltinModels`/`getBuiltinProviders` from `@sammorrowdrums/mcpi-ai/providers/all`), `registerApiProvider`/`unregisterApiProviders`/`resetApiProviders`/`getApiProvider`, `getEnvApiKey`/`findEnvKeys`, `setBedrockProviderModule`, the per-API lazy stream wrappers (`anthropicMessagesApi`, ...), and the image-generation API.
...
- API implementation modules moved from `src/providers/` to `@sammorrowdrums/mcpi-ai/api/*`, renamed by API id (`anthropic` -> `api/anthropic-messages`, `google` -> `api/google-generative-ai`, `mistral` -> `api/mistral-conversations`, `amazon-bedrock` -> `api/bedrock-converse-stream`), each exporting exactly `stream` and `streamSimple`. The old per-impl export names (`streamAnthropic`, `streamSimpleAnthropic`, ...) and legacy raw API subpaths (`./anthropic`, `./google`, `./openai-completions`, ...) are gone; import raw API implementations through `@sammorrowdrums/mcpi-ai/api/*`.
- Removed the `@sammorrowdrums/mcpi-ai/base` selective-provider entrypoint; use the root/core APIs with explicit `createModels()` collections and provider factories for isolated bundles.
...
- To keep the old global API temporarily, change imports from `@sammorrowdrums/mcpi-ai` to `@sammorrowdrums/mcpi-ai/compat`. The compat entrypoint preserves `stream`/`complete`, generated catalog reads, API registry helpers, env API-key helpers, lazy API wrappers, and image globals, but it will be removed in a future release.
...
- To call a raw API implementation directly, import from `@sammorrowdrums/mcpi-ai/api/*` and pass a compatible model plus auth/options yourself. Raw API modules export `stream` and `streamSimple`; use `.result()` on the returned stream for `complete`/`completeSimple` behavior:

  ```ts
  import { streamSimple } from "@sammorrowdrums/mcpi-ai/api/anthropic-messages";
...
- One provider factory per built-in provider under `@sammorrowdrums/mcpi-ai/providers/*` (e.g. `anthropicProvider()`, `openrouterProvider()`), plus `@sammorrowdrums/mcpi-ai/providers/all` with `builtinProviders()`/`builtinModels()` and typed `getBuiltin*` catalog reads. Generated catalogs are split per provider, so importing one provider pulls one catalog; `sideEffects` metadata makes the package tree-shakeable.
...
- Added `@sammorrowdrums/mcpi-ai/base` and direct provider registration exports for bundlers that want selective provider transports without root built-in registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
