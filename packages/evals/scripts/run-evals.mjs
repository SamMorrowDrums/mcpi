...
const artifactDirectory = process.env.MCPI_EVAL_ARTIFACT_DIR
	? resolve(packageRoot, process.env.MCPI_EVAL_ARTIFACT_DIR)
	: resolve(
			packageRoot,
			".eval",
...
if (hasCliModelSelection) {
	...
	provider = process.env.MCPI_PROVIDER?.trim() || undefined;
	model = process.env.MCPI_MODEL?.trim() || undefined;
	if (Boolean(provider) !== Boolean(model)) {
		console.error("Default model selection requires both MCPI_PROVIDER and MCPI_MODEL.");
		process.exit(1);
	}
}
...
const childEnvironment = {
	...process.env,
	MCPI_EVAL_ARTIFACT_DIR: artifactDirectory,
};
if (provider && model) {
	childEnvironment.MCPI_PROVIDER = provider;
	childEnvironment.MCPI_MODEL = model;
} else {
	delete childEnvironment.MCPI_PROVIDER;
	delete childEnvironment.MCPI_MODEL;
}
