...
import { MCPI_SESSION_SNAPSHOT_ARTIFACT, persistEvalArtifactReferences } from "./artifacts.ts";
...
async function appendHarnessRunReport(test: TestCase): Promise<void> {
	const artifactDirectory = process.env.MCPI_EVAL_ARTIFACT_DIR?.trim();
	...
	const metadata = Object.fromEntries(
		Object.entries(run.artifacts ?? {}).filter(
			([name]) => name !== "runId" && name !== MCPI_SESSION_SNAPSHOT_ARTIFACT,
		),
	);
	const record = {
		schemaVersion: 1,
