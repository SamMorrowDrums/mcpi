...
interface SourceArtifact extends TestArtifactBase {
	type: "@sammorrowdrums/mcpi-evals:source";
	runId: string;
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@sammorrowdrums/mcpi-evals:session" &&
				artifact.type !== "@sammorrowdrums/mcpi-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@sammorrowdrums/mcpi-evals:session" ? "sessions" : "sources";
