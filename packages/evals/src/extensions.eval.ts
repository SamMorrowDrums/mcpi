...
import { createMcpiCodingAgentHarness, type McpiCodingAgentInput } from "./mcpi-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasMcpiDocs: boolean;
	...
	return createMcpiCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			return {
				...
				systemPromptHasMcpiDocs: session.systemPrompt.includes("\nmcpi documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default mcpi system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default mcpi system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<McpiCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@sammorrowdrums/mcpi")) {
				failures.push("extension does not import the canonical @sammorrowdrums/mcpi package");
			}
...
const extensionHarnessTable = evalHarnessTable("mcpi extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"mcpi extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create an mcpi extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("mcpi eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasMcpiDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
