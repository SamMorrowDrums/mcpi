...
Stateful agent with tool execution and event streaming. Built on `@sammorrowdrums/mcpi-ai`.

## Installation

```bash
npm install @sammorrowdrums/mcpi-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@sammorrowdrums/mcpi-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
```typescript
declare module "@sammorrowdrums/mcpi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@sammorrowdrums/mcpi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@sammorrowdrums/mcpi-agent-core";
