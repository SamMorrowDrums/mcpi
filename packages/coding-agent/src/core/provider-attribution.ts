import type { Api, Model, ProviderHeaders } from "@sammorrowdrums/mcpi-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://github.com/SamMorrowDrums/mcpi",
			"X-OpenRouter-Title": "mcpi",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "mcpi",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "mcpi-coding-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "mcpi" };
}
