...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as MCPI_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.MCPI_SESSION_ID = metadata.id;
						execution.env.MCPI_SESSION_FILE = sessionFile ?? "";
						execution.env.MCPI_PROVIDER = model.provider;
						execution.env.MCPI_MODEL = model.id;
						execution.env.MCPI_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
