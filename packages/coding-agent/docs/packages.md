...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@sammorrowdrums/mcpi-ai`, `@sammorrowdrums/mcpi-agent-core`, `@sammorrowdrums/mcpi`, `@sammorrowdrums/mcpi-tui`, `typebox`.
