...
- Added `deferred` to the extension `ToolDefinition`, so `pi.registerTool({ ..., deferred: true })` withholds the tool's schema from the model until a load point rather than sending it on the first request. This is visibility, not authorization: the tool stays registered and active, remains in `pi.getAllTools()` and `pi.getActiveTools()`, and is still dispatchable, so a direct call executes normally. Because no tool is removed and no prompt metadata changes, the system prompt is not rebuilt and the cached prompt prefix is preserved. Registration deferral composes with `pi.setActiveTools()`: a loader tool naming a deferred tool loads its schema at that tool-result position, and once the model has called a tool it stays immediate. Registration deferral needs a turn-zero anchor: Anthropic sends the full definition with `defer_loading: true` and OpenAI's `gpt-5.4` and later send it with `defer_loading: true` alongside a hosted `tool_search` entry, so in both cases the tool stays part of the request and a call still resolves. A model with neither anchor sends registration-deferred schemas up front with a `deferred_tools_unsupported` diagnostic rather than let deferral become a dispatch gate. A deferred tool is discovered either through the tools you leave immediate — a loader, index, or search tool whose result names tools via `addedToolNames` — or through a provider's own tool search where mcpi sends one. Keep at least one tool immediate if your provider has no such search. See [Deferring a tool from turn zero](docs/extensions.md#deferring-a-tool-from-turn-zero).
- Added `namespace` to the extension `ToolDefinition`, an optional `{ name, description }` group for deferred tools. On the OpenAI Responses paths a flat deferred tool still shows the model its name and description and withholds only the parameter schema, so an extension proxying twenty MCP tools still paid for twenty names and descriptions on turn zero. Tools sharing a namespace are sent as one group entry and the model sees only the group summary until it searches. Pass the MCP server's own name and description. Namespaces do not affect dispatch — a tool is still called by its unique registered name — and are ignored on APIs that do not model them.

### Fixed

- Fixed a tool registered with `deferred: true` being unreachable when no loader tool ever named it. Its schema was withheld from turn zero and the only load point was a `pi.setActiveTools()` marker, so a tool no skill named stayed invisible for the whole session. mcpi now offers the provider's own search tool alongside the deferred definitions — Anthropic's server-side search on Anthropic-family providers, OpenAI's hosted `tool_search` on `gpt-5.4` and later — so the model can find the tool itself and then call it directly. Where search is unavailable those schemas are sent up front with a `deferred_tools_unsupported` diagnostic rather than being withheld with no way back. See [Finding a deferred tool that no skill loads](docs/extensions.md#finding-a-deferred-tool-that-no-skill-loads).
- Fixed registration-deferred tools being expanded on OpenAI models that can in fact withhold them. `deferred: true` was treated as unsupported on every OpenAI-family model, on the grounds that they can only load tools from a tool-result-anchored item, so an extension proxying MCP tools paid for every schema on turn zero there. `gpt-5.4` and later serve a hosted tool search over deferred definitions, which is a turn-zero route to a withheld schema, so those models now defer for real. Models without it are unchanged.
...
- The mcpi 0.85.0 release is based on upstream `pi` v0.84.2; mcpi versions are independent.
