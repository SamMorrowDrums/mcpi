...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.MCPI_TELEMETRY;
		delete process.env.MCPI_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.MCPI_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/SamMorrowDrums/mcpi");
		expect(headers?.["X-OpenRouter-Title"]).toBe("mcpi");
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe("mcpi");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("mcpi");
	});
