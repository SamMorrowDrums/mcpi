...
const originalOffline = process.env.MCPI_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.MCPI_OFFLINE;
	else process.env.MCPI_OFFLINE = originalOffline;
});
