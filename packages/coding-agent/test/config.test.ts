...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@sammorrowdrums/mcpi")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @sammorrowdrums/mcpi",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getUpdateInstruction("@sammorrowdrums/mcpi")).toBe(
			"Update @sammorrowdrums/mcpi using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@sammorrowdrums/mcpi"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @sammorrowdrums/mcpi`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@sammorrowdrums/mcpi", undefined, {
			packageName: "@sammorrowdrums/mcpi",
			installSpec: "@sammorrowdrums/mcpi@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@sammorrowdrums/mcpi@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @sammorrowdrums/mcpi@1.2.3`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@old-scope/mcpi"],
					display: `npm --prefix ${prefix} uninstall -g @old-scope/mcpi`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@sammorrowdrums/mcpi", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @sammorrowdrums/mcpi`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@sammorrowdrums/mcpi")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @sammorrowdrums/mcpi",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@sammorrowdrums/mcpi"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @sammorrowdrums/mcpi",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@old-scope/mcpi"],
					display: "pnpm remove -g @old-scope/mcpi",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/pi"],
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@old-scope/mcpi"],
					display: "yarn global remove @old-scope/mcpi",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@old-scope/mcpi"],
					display: "bun uninstall -g @old-scope/mcpi",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/pi"],
