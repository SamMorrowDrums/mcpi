...
describe.skipIf(isWindows)("session environment in the bash tool", () => {
	...
	beforeEach(() => {
		testDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-session-env-"));
	});

	afterEach(() => {
		fs.rmSync(testDir, { recursive: true, force: true });
		delete process.env.PI_SESSION_ENV_INHERITED;
	});

	it("passes session variables to spawned commands", async () => {
		const sessionEnv = new Map<string, string | null>([["PI_SESSION_ENV_TEST", "from-session"]]);
		const bash = createBashTool(testDir, { spawnHook: spawnHookFor(sessionEnv) });

		const result = await bash.execute("call-1", { command: 'echo "value=$PI_SESSION_ENV_TEST"' });
	...
	it("masks an inherited variable when the overlay value is null", async () => {
		process.env.PI_SESSION_ENV_INHERITED = "leaked";
		const sessionEnv = new Map<string, string | null>([["PI_SESSION_ENV_INHERITED", null]]);
		const bash = createBashTool(testDir, { spawnHook: spawnHookFor(sessionEnv) });

		const result = await bash.execute("call-2", {
			command:
				'if env | grep -q "^PI_SESSION_ENV_INHERITED="; then echo "value=present"; else echo "value=absent"; fi',
		});
	...
	it("does not mutate process.env", async () => {
		...
		expect(process.env.PI_SESSION_ENV_TEST).toBeUndefined();
	});
	...
	it("still runs a user-supplied spawnHook, which can override the session value", async () => {
		...
		const bash = createBashTool(testDir, {
			spawnHook: (context) => {
				...
				sawSessionValue = withSession.env.PI_SESSION_ENV_TEST;
				// ...then a user hook observes and may override it.
				return { ...withSession, env: { ...withSession.env, PI_SESSION_ENV_TEST: "from-user-hook" } };
			},
		});

		const result = await bash.execute("call-4", { command: 'echo "value=$PI_SESSION_ENV_TEST"' });
...
describe.skipIf(isWindows)("session environment in execCommand", () => {
	...
	it("passes the session overlay to the child process", async () => {
		const result = await execCommand(
			"sh",
			["-c", 'printf "%s" "$PI_SESSION_ENV_TEST"'],
			process.cwd(),
			undefined,
			new Map([["PI_SESSION_ENV_TEST", "from-session"]]),
		);
	...
	it("gives per-call options.env precedence over the session overlay", async () => {
		const result = await execCommand(
			...
			["-c", 'printf "%s" "$PI_SESSION_ENV_TEST"'],
			process.cwd(),
			{ env: { PI_SESSION_ENV_TEST: "from-call" } },
			new Map([["PI_SESSION_ENV_TEST", "from-session"]]),
		);
	...
	it("masks an inherited variable when the overlay value is null", async () => {
		...
		const result = await execCommand(
			"sh",
			["-c", 'if env | grep -q "^PI_SESSION_ENV_INHERITED="; then printf present; else printf absent; fi'],
			process.cwd(),
			undefined,
			new Map([["PI_SESSION_ENV_INHERITED", null]]),
		);

		expect(result.stdout).toBe("absent");
		// Masking is scoped to the child; pi's own environment is unchanged.
		expect(process.env.PI_SESSION_ENV_INHERITED).toBe("leaked");
	});
});
...
describe("pi.setEnv / pi.unsetEnv extension API", () => {
	...
	beforeEach(async () => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-session-env-ext-"));
	...
	pi.setEnv("PI_SESSION_ENV_TEST", "from-extension");
}`);

		expect(runner.getSessionEnv().get("PI_SESSION_ENV_TEST")).toBe("from-extension");
	});

	it("records unsetEnv as an explicit mask", async () => {
		const runner = await loadRunner(`export default function(pi) {
	pi.setEnv("PI_SESSION_ENV_TEST", "from-extension");
	pi.unsetEnv("PI_SESSION_ENV_TEST");
	pi.unsetEnv("PI_SESSION_ENV_INHERITED");
}`);

		const sessionEnv = runner.getSessionEnv();
		expect(sessionEnv.get("PI_SESSION_ENV_TEST")).toBeNull();
		expect(sessionEnv.get("PI_SESSION_ENV_INHERITED")).toBeNull();
	});
	...
	it.skipIf(isWindows)("reaches a command spawned through the bash tool", async () => {
		const runner = await loadRunner(`export default function(pi) {
	pi.setEnv("PI_SESSION_ENV_TEST", "end-to-end");
}`);
		const bash = createBashTool(tempDir, { spawnHook: spawnHookFor(runner.getSessionEnv()) });

		const result = await bash.execute("call-e2e", { command: 'echo "value=$PI_SESSION_ENV_TEST"' });
