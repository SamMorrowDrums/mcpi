...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".config", "mcpi"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".mcpi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".mcpi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".mcpi"), { recursive: true });
			writeFileSync(join(cwd, ".mcpi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".mcpi"), { recursive: true, force: true });
