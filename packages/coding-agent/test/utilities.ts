...
import type { OAuthCredentials } from "@sammorrowdrums/mcpi-ai";
import { getModel, streamSimple } from "@sammorrowdrums/mcpi-ai/compat";
import { builtinProviders } from "@sammorrowdrums/mcpi-ai/providers/all";
import { getAgentDir, getAuthPath } from "../src/config.ts";
...
// ============================================================================
// OAuth API key resolution from the mcpi config directory
// ============================================================================
...
/**
 * Resolve API key for a provider from mcpi's auth.json
 *
...
/**
 * Check if a provider has credentials in mcpi's auth.json
 */
...
/** Path to the real mcpi config directory */
export const MCPI_AGENT_DIR = getAgentDir();

/**
 * Get an AuthStorage instance backed by mcpi's auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `mcpi-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
