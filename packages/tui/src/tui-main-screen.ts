...
export class TuiMainScreen extends TuiBase implements TUI {
	...
	protected doRender(): void {
		...
		const debugRedraw = process.env.MCPI_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(this.logDirectory, "mcpi-debug.log");
		...
		// Configurable via setClearOnShrink() or MCPI_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(this.logDirectory, "mcpi-crash.log");
				const crashData = [
		...
		if (process.env.MCPI_TUI_DEBUG === "1") {
